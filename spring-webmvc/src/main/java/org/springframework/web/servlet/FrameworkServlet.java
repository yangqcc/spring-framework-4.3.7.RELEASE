...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
    ...
    /**
     * 返回客户端配置的 上下文 class,如果没有配置,默认为XmlWebApplicationContext.class
    ...
    /**
     * 当前Servlet初始化WebApplicationContext,委托{@link #createWebApplicationContext}
     * 进行实际上下文的创建,可被子类覆盖
     * <p>
    ...
    protected WebApplicationContext initWebApplicationContext() {
        //获取ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE的值,该值存放在ServletContext
        //保存的是WebApplicationContext的子类实例
        WebApplicationContext rootContext = WebApplicationContextUtils.getWebApplicationContext(getServletContext());
        WebApplicationContext wac = null;

        if (this.webApplicationContext != null) {
            // A context instance was injected at construction time -> use it
            //一个context实例在构造函数中被创建
        ...
        if (wac == null) {
            //通过contextAttribute来查找ServletContext中对应的属性
        ...
        //如果以上两种方式没有找到WebApplicationContext,那只能重新创建实例了
        if (wac == null) {
    ...
    protected WebApplicationContext createWebApplicationContext(ApplicationContext parent) {
        ...
        //通过反射实例化contextClass类
        ConfigurableWebApplicationContext wac =
                (ConfigurableWebApplicationContext) BeanUtils.instantiateClass(contextClass);

        wac.setEnvironment(getEnvironment());
        //parent为在ContextLoaderListener中创建的实例
        //在ContextLoaderListener记载的时候初始化的WebApplicationContext类型实例
    ...
    /**
     * 当前servlet实例化WebApplicationContext
     * Instantiate the WebApplicationContext for this servlet, either a default
     * {@link org.springframework.web.context.support.XmlWebApplicationContext}
     * or a {@link #setContextClass custom context class}, if set.
